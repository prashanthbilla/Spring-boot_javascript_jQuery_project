...
public class UserRepository implements IUserRepository {
    ...
    public UserModel getByUserId(String userId) {
        try {
            ...
            return jdbcTemplate.queryForObject("select * from register where userId=?", (rs, rowNum) -> {
                return new UserModel(rs.getLong("id"), rs.getString("userId"), rs.getString("name"),
                        rs.getString("password"), rs.getString("phoneNum"));
            }, userId);
    ...
    @Override
    public String saveFile(PrimeMinister primeMinister) {
            try {
                int i = jdbcTemplate.update("insert into prime_minister_of_india (id, name, photo) values(?,?,?)", primeMinister.getId(),
                        primeMinister.getName(), primeMinister.getPhoto());
                if (i == 1)
                    return "Inserted...Successfully";
                return null;
            } catch (Exception e) {
    ...
    public byte[] getFile(int id) {
        ...
        try {
            primeMinister = jdbcTemplate.queryForObject("select * from prime_minister_of_india where id=?", (rs, rowNum) -> {
                return new PrimeMinister(rs.getInt("id"), rs.getString("name"),
                        rs.getBytes("photo"));
            }, id);

            assert primeMinister != null;
            return primeMinister.getPhoto();
        } catch (Exception e) {
            return null;
        }
